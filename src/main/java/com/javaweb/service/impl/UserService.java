...
public class UserService implements IUserService {
    ...
    @Override
    @Transactional
    public RegisterDTO register(RegisterDTO newUser) {
        UserEntity activeUser = userRepository.findOneByUserNameAndStatus(newUser.getUserName(), 1);
        if (activeUser != null) {
        ...
        if (role == null) {
            throw new MyException("Không có role 'user'.");
        }

        UserEntity userEntity = userConverter.convertRegisterToEntity(newUser);
        userEntity.setPassword(passwordEncoder.encode(newUser.getPassword()));
        userEntity.setStatus(1);
        userEntity.setRoles(Collections.singletonList(role));
        return userConverter.convertRegisterDTO(userRepository.save(userEntity));
    }
